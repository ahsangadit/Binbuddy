...
function change_coupon_discount($discount, $discounting_amount, $cart_item, $single, $coupon)
{
...
function save_coupon_on_checkout( $order_id ) {
	...
	$user_data = array(
		...
		'meta_value' => $order->billing_address_2
	);

	if($order->billing_address_2 != '') {
		$has_user = get_users( $user_data );
		if ( ! empty( $has_user ) ) {
			$ref_user_id = $has_user[0]->data->ID;
			$ref_coupon  = get_user_meta( $ref_user_id, 'user_coupon', true );
			$order->apply_coupon( $ref_coupon[0] );
		}
	}

	if( $user == false && $email == false && (empty($has_user) == true) ){
...
include dirname( __FILE__ ) . '/templates/sales_coupons.php';

add_filter( 'woocommerce_checkout_fields', 'override_billing_checkout_fields', 10, 1 );

function override_billing_checkout_fields( $fields ) {
	$fields['billing']['billing_address_1']['placeholder'] = 'Your Addres';
	$fields['billing']['billing_address_2']['placeholder'] = 'Referrence Address';
	return $fields;
}
