...
/**
 * Clear FCGI Cache via Hummingbird.
 */
add_action(
	'plugins_loaded',
	function() {
		if ( defined( 'WPHB_VERSION' ) && version_compare( WPHB_VERSION, '2.6.1', '<=' ) ) {
			add_action(
				'wphb_clear_cache_url',
				function( $dir ) {
					if ( 'production' !== $_SERVER['WPMUDEV_HOSTING_ENV'] ) {
						return;
					}
