...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	...
	/**
	 * Get siteurl option  value.
	 */
	class Custom_WP_Doctor_Get_Siteurl extends runcommand\Doctor\Checks\Check {
		// Main function.
		public function run() {
			...
			$message = '';
			$siteurl = WP_CLI::runcommand(
				'option get siteurl --format=json',
				Custom_WP_Doctor_Helper::runcommand_options()
			);
			// Check if siteurl contains extra line.
			if ( strstr( $siteurl, "\n" ) ) {
				$this->set_status( CUSTOM_WP_DOCTOR_ERROR );
				$message = 'New line character found.' . ' ';
			}
			// Return message.
			$this->set_message( $message . $siteurl );
		}
	}
	/**
	 * Get home option value.
	 */
	class Custom_WP_Doctor_Get_Home extends runcommand\Doctor\Checks\Check {
		// Main function.
		public function run() {
			// Set status as success by default.
			$this->set_status( CUSTOM_WP_DOCTOR_SUCCESS );
			...
			$home = WP_CLI::runcommand(
				'option get home --format=json',
				Custom_WP_Doctor_Helper::runcommand_options()
			);
			// Check if home contains extra line.
			if ( strstr( $home, "\n" ) ) {
				$this->set_status( CUSTOM_WP_DOCTOR_ERROR );
				$message = 'New line character found.' . ' ';
			}
			// Return message.
			$this->set_message( $message . $home );
		}
	}
	/**
	 * Get upload_path option value.
	 */
	class Custom_WP_Doctor_Get_Upload_Path extends runcommand\Doctor\Checks\Check {
		// Main function.
		public function run() {
   		...
   		$env = 'staging';
			...
			// Set status as success by default.
			$this->set_status( CUSTOM_WP_DOCTOR_SUCCESS );
			$upload_path = WP_CLI::runcommand(
				'option get upload_path --format=json',
				Custom_WP_Doctor_Helper::runcommand_options()
			);
			// Check if upload path is empty
			if ( ! empty( $upload_path ) ) {
				$this->set_status( CUSTOM_WP_DOCTOR_WARNING );
				} else {
				if ( 'staging' === $env ) {
 				$upload_path = '/var/web/staging/public_html/wp-content/uploads';
				} else {
 				$upload_path = '/var/web/site/public_html/wp-content/uploads';
				}
			}
			// Return message.
			$this->set_message( $upload_path );
		}
	}
